...
**4. `BUILD` drives the localStorage key.** `STORAGE_KEY` is `vision8-homepage-editor-${BUILD}`. The editor merges saved records over source defaults, so a stale key can pin old media and mask a correct deploy. **Bump `BUILD` in `app/homepage-view-v1.10.3.tsx` whenever defaults change.** The page titles no longer carry a version, deliberately, because they went stale and misled diagnosis.

There are **two** `BUILD` constants and both must move together. `homepage-view-v1.10.3.tsx` stamps the homepage and keys the homepage editor; `app/portfolio-shell.tsx` stamps the footer on every other route and keys the photography editor. Bumping only the first leaves nine routes on the old number and fails nine route tests. The number is also pinned three times in `tests/rendered-html.test.mjs`, twice as a regex with escaped dots, so a plain find-and-replace on `v1.11.71` misses them. Persistence is guarded by a `dirty` ref, so loads and reloads no longer write unprompted, but bump anyway.
