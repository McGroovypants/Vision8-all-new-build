...
type GlobalEditorSettings = Pick<EditorStyles, "mediaOpacity" | "mediaFadeMs" | "googleFontUrl">;
...
function NudgeControl({
  ...
  return (
    <fieldset className="nudge-control">
      <legend>{label}</legend>
      <div className="nudge-pad" aria-label={`${label} position controls`}>
        <button type="button" onClick={() => onChange(x, y - 1)} aria-label={`Move ${label} up one pixel`}>↑</button>
        <button type="button" onClick={() => onChange(x - 1, y)} aria-label={`Move ${label} left one pixel`}>←</button>
        <span aria-hidden="true">1px</span>
        <button type="button" onClick={() => onChange(x + 1, y)} aria-label={`Move ${label} right one pixel`}>→</button>
        <button type="button" onClick={() => onChange(x, y + 1)} aria-label={`Move ${label} down one pixel`}>↓</button>
      </div>
  ...
  return (
    <aside className="editor-panel" aria-label="Vision8 homepage editor">
      ...
      <section className="editor-section">
        ...
        <p className="editor-note">For an installed font, enter its exact family name below. For Google Fonts, paste its stylesheet URL here, then use its family name in the relevant text group.</p>
      </section>
      ...
      <section className="editor-section">
        ...
        <datalist id="font-suggestions">
          ...
          <option value={'"Avenir Next", Avenir, sans-serif'} />
          ...
          <fieldset key={key} className="style-control">
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => onStyleChange(key, { fontFamily: event.target.value })}
              />
            </label>
            <label>
              Size
        ...
        <p className="editor-note">These controls change visual appearance only. The page keeps one semantic H1 and a clear heading structure for SEO.</p>
      </section>
...
export function HomepageV1100() {
  ...
  return (
    <main className="v1100-shell" style={styleVars}>
      ...
      <section className={`home-stage active-${active.id}${isCycling ? " fan-cycle" : ""}`}>
        ...
        <div className="stage-copy" key={`copy-${active.id}`} aria-live="polite">
          <p className="editable-offset" style={textOffsetStyle(active, "kicker")}>{active.kicker}</p>
          <h1 className="editable-offset" style={textOffsetStyle(active, "headline")}>{active.headline}</h1>
          <div className="copy-row">
            <span className="editable-offset" style={textOffsetStyle(active, "body")}>{active.body}</span>
